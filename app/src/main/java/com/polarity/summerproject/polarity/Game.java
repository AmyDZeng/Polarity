...
public class Game{
    ...
    private int tapThreshold = 20; // eg. must be within 5 px to be a TAP
    Typeface font;
    ...
    public void draw(Canvas canvas){    // DRAW OBSTACLE IN ITS OWN FUNCTION? probably
        ...
        paint.setTypeface(MainActivity.myCustomFont);
        ...
        switch(action) {
            case(MotionEvent.ACTION_DOWN) :
                ...
                if(me.getY() > obsHolder.btn.y && me.getX() < obsHolder.btn.width){
                    ...
                    obsHolder.playerChange(); // change to white
            ...
            case(MotionEvent.ACTION_UP) :
                ...
                if(me.getPointerId(0) == btnPointerId) { // button released TODO: ??? check over this part
                    //obsHolder.playerChange(false); // should have no effect
                    btnPointerId = -1;
                }else{ // swipe released
                    newX = me.getX();
                    newY = me.getY();
            ...
            case(MotionEvent.ACTION_POINTER_UP) :
                ...
                if(btnPointerId == me.getPointerId(MotionEventCompat.getActionIndex(me))){
                    // button released
                    //obsHolder.playerChange(false); // action_up should have no effect on change
                    btnPointerId = -1;
                }else{
                    // swipe released
