...
public class ObstacleHolder {
    ...
    public ObstacleHolder(Screen screen,int size, int speed){
        ...
        for(int i = 0; i < rows; i++){
            for(int j = 0; j < cols; j++){
                ...
                obsHolder[j + cols*i] = new Obstacle(Color.BLACK, obsSize, j*obsSize + sideBoarderSize, i*obsSize/* + topBoarderSize*/ /*- screen.getHeight()*/ + topBoarderSize, this.speed);
            }
        }
    }
    ...
    public void playerChange(){
        // swap polarity button colours
        if(btn.imageIndex == 1)btn.imageIndex = 0;
        else if(btn.imageIndex == 0) btn.imageIndex = 1;

        if(player.change == true)player.change = false;
        else if(player.change == false)player.change = true;
    }
    /*
